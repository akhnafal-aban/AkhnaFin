...
**AI = OpenRouter satu-call (PLAN-006, menggantikan Foundation Models):** `google/gemma-4-26b-a4b-it:free` (MoE, ~3.8B aktif/token — paling ringan di kandidat gratis; native `response_format json_schema`) — teks/gambar resi → fakta + kategori dalam SATU panggilan (kategori dibantu daftar kategori user + `CategorySignal`). Awalnya 2-stage (Nemotron→gpt-oss) diukur live **23.2s**; collapse ke 1 model dgn `reasoning.effort:"low"` (motong budget reasoning, model non-wajib-reasoning) turunkan ke **7.9s**. **Gotcha:** `provider.require_parameters:true` HANYA saat model dukung `response_format` (cek `GET /api/v1/models` field `supported_parameters` — Nemotron omni `:free` TAK dukung, `gpt-oss-120b:free` malah TIDAK ADA slug-nya). API key di Keychain (`OpenRouterKeyStore`), diisi user via Pengaturan; tanpa key → graceful state. Personalisasi kategori = knowledge-graph mini `CategorySignal` (edge berbobot di SwiftData; commit jalur AI merekam, prompt stage-2 membawa asosiasi relevan).
